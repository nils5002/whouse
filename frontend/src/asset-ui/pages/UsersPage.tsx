...
export function UsersPage({ users }: { users: UserItem[] }) {
  return (
    <section className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-brand-700">Benutzer / Rollen</p>
          <h2 className="mt-1 text-2xl font-semibold tracking-tight text-slate-900 sm:text-3xl">Teamzugriff</h2>
          <p className="mt-1 text-sm text-slate-500">
            Rechteverwaltung fuer Admin, Logistik, Mitarbeiter, Event-Team und Nur-Lesen.
          </p>
        </div>
          ...
          <table className="w-full min-w-[760px] border-separate border-spacing-y-2 text-sm">
            <thead>
              <tr className="text-left text-xs uppercase tracking-wide text-slate-500">
                ...
                <th className="px-3 py-2">Letzte Aktivitaet</th>
        ...
        <div className="grid gap-3 md:hidden">
          {users.map((user) => (
            <article key={`mobile-${user.id}`} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
              ...
              <p className="mt-2 text-xs text-slate-500">Letzte Aktivitaet: {user.lastActive}</p>
            </article>
          ))}
        </div>
      </article>
